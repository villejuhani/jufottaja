...
await ProcessFile(filePath, outputFilePath, headers);
...
async Task ProcessFile(string filePath, string outputFilePath, Dictionary<HeaderType, string[]> headers)
{
    ...
    catch (Exception e)
    {
        ...
        {
            Console.WriteLine($"The file {outputFilePath} is currently being used by another process. Please close any programs using it and try again.");
            Environment.Exit(1);
        }
    ...
    Console.WriteLine($"  - Multiple possible channels found in Jufo: {multipleJufoChannelPossibilites} (look the correct one up manually)");
    Console.WriteLine($"  - Rows with all empty {string.Join(", ", headers.SelectMany(kvp => kvp.Value))} column(s): {emptyHeaderColumnRows}");
